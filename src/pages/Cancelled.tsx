  ...
  const fetchCancelledJobs = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.error('Failed to load cancelled jobs.');
        return;
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center mb-6">
        <div className="relative">
          <XCircle className={`absolute left-3 top-1/2 -translate-y-1/2 ${mutedTextColor}`} size={20} />
          <input
            type="text"
            placeholder="Search cancelled jobs..."
            value={searchTerm}
          ...
          <table className="min-w-full divide-y divide-gray-700">
            <thead className={`${sectionBg}`}>
              <tr>
                ...
                <th className={`px-6 py-4 text-left text-xs font-medium uppercase tracking-wider ${headerTextColor}`}>Cancelled</th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                filteredJobs.map((job) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className={`font-medium ${textColor}`}>{formatDate(job.cancelled_at)}</div>
                    </td>
                  </tr>
                ))
              )}
